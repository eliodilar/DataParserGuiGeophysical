...
class DrawingXYZ:
  def __init__(self):
    ...
    def WatchRowDatCSV(self):
      ...
      filepath = askopenfilename(title="Ouvrir une image",filetypes=[('all files','.*')]) _chemin=filepath
...
boutonCSVWatch=Button(FrameTraitement, text="Watch CSV Data Formating", 
command=Classe_Drawing.WatchRowDatCSV)
boutonCSVWatch.pack()
# bouton d'ouverture csv
boutonCSV=Button(FrameTraitement, text="OpenCSVFile", 
command=Classe_Drawing.CSVLoader)
boutonCSV.pack()
# bouton d'ouverture csv
boutonDraw=Button(FrameTraitement, text="Traçer la courbe de X / T", 
command=Classe_Drawing.DrawPlotXT)
boutonDraw.pack()
# Input X col selection number
entree = Entry(FrameColonne, width=30)
entree.pack()
# bouton de validation selection colonne
bouton=Button(FrameColonne, text="Col validation selector", 
command=Classe_Drawing.SetIdColX(entree.get()))
bouton.pack()
fenetre.mainloop()
#-------------------------------END_MAIN_LOOP-----------------------------------
------------------
